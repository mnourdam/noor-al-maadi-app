...
describe("emblem local-first resolution v2", () => {
  it("every registry id resolves to a bundled local WebP as first candidate", () => {
    // Legacy ids resolve through LEGACY_PREMIUM_EMBLEM_ALIASES, exactly as the
    // render path does, so every stored avatar_id must land on local artwork.
    const offenders = EMBLEM_REGISTRY.filter(({ id }) => {
      const first = emblemSourceCandidates(resolveProfileEmblem(id).record, 512)[0];
  ...
  it("the bundled pack backs every first candidate on disk", () => {
    ...
    const missing = EMBLEM_REGISTRY.map(
      ({ id }) => emblemSourceCandidates(resolveProfileEmblem(id).record, 512)[0],
    )
