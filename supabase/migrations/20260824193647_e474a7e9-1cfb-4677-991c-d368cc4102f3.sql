...
BEGIN
  ...
  SELECT count(*)::int,
         count(*) FILTER (WHERE a.wait_event IS NOT NULL)::int,
         count(*) FILTER (WHERE a.wait_event_type = 'Lock')::int,
         COALESCE(max(EXTRACT(EPOCH FROM (now() - a.query_start)))
                  FILTER (WHERE a.state = 'active' AND a.pid <> pg_backend_pid()), 0)::numeric
    INTO v_connections, v_waiting, v_waiting_lock, v_longest_query
