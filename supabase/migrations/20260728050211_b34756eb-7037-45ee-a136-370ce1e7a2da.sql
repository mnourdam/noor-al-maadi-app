...
BEGIN
  ...
  END IF;
  IF v_write_mode NOT IN ('draft','publish') THEN
    RAISE EXCEPTION 'invalid write_mode';
  END IF;
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_payload->'campaigns') AS e(value) LOOP
    v_total := v_total + 1;
    v_errors := ARRAY[]::text[];
    ...
    v_old_ch := ARRAY[]::text[];
    v_new_ch := ARRAY[]::text[];
    v_old_act := ARRAY[]::text[];
    v_new_act := ARRAY[]::text[];
    v_removed_ch := ARRAY[]::text[];
    v_added_ch := ARRAY[]::text[];
    v_removed_act := ARRAY[]::text[];
    v_added_act := ARRAY[]::text[];
    v_matched_by := NULL;
    ...
    v_data  := CASE
      WHEN jsonb_typeof(v_item->'data') = 'object' THEN v_item->'data'
    ...
    IF v_id IS NULL AND v_slug IS NULL THEN
      v_errors := array_append(v_errors, 'العنصر لا يحتوي id ولا slug.');
    END IF;
    IF v_data IS NULL THEN
      v_errors := array_append(v_errors, 'الحقل data مفقود أو ليس كائنًا.');
    ELSIF jsonb_typeof(v_data->'chapters') <> 'array' THEN
      v_errors := array_append(v_errors, 'data.chapters يجب أن يكون مصفوفة.');
    END IF;
    IF v_id IS NOT NULL AND v_id LIKE 'div\_%' THEN
      v_errors := array_append(v_errors, 'الفواصل التنظيمية (div_*) لا تُستورد.');
    END IF;
    ...
    IF v_id IS NOT NULL THEN
      ...
      END IF;
    END IF;
    IF NOT v_has_existing AND v_slug IS NOT NULL THEN
      SELECT * INTO v_existing FROM public.admin_campaigns WHERE slug = v_slug;
      IF FOUND THEN
        v_matched_by := 'slug';
        v_id := COALESCE(v_id, v_existing.id);
        v_has_existing := true;
      END IF;
    END IF;

    IF NOT v_has_existing AND v_title IS NULL THEN
      v_errors := array_append(v_errors, 'حملة جديدة بدون عنوان.');
    END IF;

    IF v_has_existing AND v_data IS NOT NULL AND jsonb_typeof(v_data->'chapters') = 'array' THEN
      SELECT COALESCE(array_agg(ch->>'id'), ARRAY[]::text[]) INTO v_old_ch
        FROM jsonb_array_elements(COALESCE(v_existing.data->'chapters','[]'::jsonb)) ch
       WHERE ch->>'id' IS NOT NULL;
      SELECT COALESCE(array_agg(ch->>'id'), ARRAY[]::text[]) INTO v_new_ch
        FROM jsonb_array_elements(v_data->'chapters') ch
       WHERE ch->>'id' IS NOT NULL;
      SELECT COALESCE(array_agg(a->>'id'), ARRAY[]::text[]) INTO v_old_act
        FROM jsonb_array_elements(COALESCE(v_existing.data->'chapters','[]'::jsonb)) ch,
             jsonb_array_elements(COALESCE(ch->'activities','[]'::jsonb)) a
       WHERE a->>'id' IS NOT NULL;
      SELECT COALESCE(array_agg(a->>'id'), ARRAY[]::text[]) INTO v_new_act
        FROM jsonb_array_elements(v_data->'chapters') ch,
             jsonb_array_elements(COALESCE(ch->'activities','[]'::jsonb)) a
       WHERE a->>'id' IS NOT NULL;

      SELECT COALESCE(array_agg(x), ARRAY[]::text[]) INTO v_removed_ch FROM unnest(v_old_ch) x WHERE NOT (x = ANY(v_new_ch));
      SELECT COALESCE(array_agg(x), ARRAY[]::text[]) INTO v_added_ch   FROM unnest(v_new_ch) x WHERE NOT (x = ANY(v_old_ch));
      SELECT COALESCE(array_agg(x), ARRAY[]::text[]) INTO v_removed_act FROM unnest(v_old_act) x WHERE NOT (x = ANY(v_new_act));
      SELECT COALESCE(array_agg(x), ARRAY[]::text[]) INTO v_added_act   FROM unnest(v_new_act) x WHERE NOT (x = ANY(v_old_act));

      IF array_length(v_removed_ch,1) > 0 OR array_length(v_removed_act,1) > 0 THEN
        IF v_allow_removals THEN
          v_warnings := array_append(v_warnings, format('سيتم حذف %s فصلًا و%s نشاطًا.',
            COALESCE(array_length(v_removed_ch,1),0), COALESCE(array_length(v_removed_act,1),0)));
        ELSE
          v_errors := array_append(v_errors, format('الملف يحذف %s فصلًا و%s نشاطًا — فعّل السماح بالحذف للمتابعة.',
            COALESCE(array_length(v_removed_ch,1),0), COALESCE(array_length(v_removed_act,1),0)));
        END IF;
      END IF;
    END IF;
    ...
    IF v_has_existing THEN
      IF v_title IS NOT NULL AND v_title IS DISTINCT FROM v_existing.title THEN
        v_fields := array_append(v_fields, 'title');
      END IF;
      IF v_slug IS NOT NULL AND v_slug IS DISTINCT FROM v_existing.slug THEN
        v_fields := array_append(v_fields, 'slug');
      END IF;
      ...
      THEN
        v_fields := array_append(v_fields, 'data');
      END IF;
      IF v_ka IS NOT NULL AND (
        ...
        v_fields := array_append(v_fields, 'key_art');
      END IF;
    END IF;
    ...
    v_items := v_items || jsonb_build_array(jsonb_build_object(
      'action', v_action,
      'id', COALESCE(v_id, CASE WHEN v_has_existing THEN v_existing.id ELSE NULL END),
      'slug', COALESCE(v_slug, CASE WHEN v_has_existing THEN v_existing.slug ELSE NULL END),
      'title', COALESCE(v_title, CASE WHEN v_has_existing THEN v_existing.title ELSE NULL END),
      ...
      'counts', jsonb_build_object(
        'chapters', COALESCE(jsonb_array_length(COALESCE(v_data->'chapters','[]'::jsonb)), 0),
        'activities', COALESCE((
