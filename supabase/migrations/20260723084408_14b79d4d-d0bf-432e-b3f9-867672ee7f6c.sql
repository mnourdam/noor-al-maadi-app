...
BEGIN
  ...
  -- Type contract notes:
  --   stories.id                  TEXT   (stable slug-style story id)
  --   story_scenes.story_id       TEXT
  --   story_media.story_id        TEXT
  --   user_story_progress.story_id      TEXT
  --   user_story_completions.story_id   TEXT
  --   social_comments.anchor_id   UUID   (frozen social design)
  --   social_reactions.anchor_id  UUID   (frozen social design)
  -- Story PKs join by TEXT natively. The social boundary is UUID by design,
  -- so we cast anchor_id::text ONLY at that boundary. Non-UUID story ids
  -- simply match zero social rows, which is the correct semantics.
