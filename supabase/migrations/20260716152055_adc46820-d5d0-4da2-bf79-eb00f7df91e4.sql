...
-- ============================================================
-- Phase 5 close-out — campaign import compatibility fixes.
-- Fix 1: preserve admin-owned ordering metadata across imports.
-- Fix 2: populate last_editor_email like editor/publish flow.
-- Fix 3: refuse to unarchive archived campaigns unless opted-in.
-- Fix 5 (client): notifyContentInvalidated — handled in wizard.
-- ============================================================

-- ---------- Fix 1: stable-ID merge preserves ordering keys ----------
...
DECLARE
  ...
  v_new_acts JSONB;
  v_matched_act JSONB;
  -- Admin-owned keys that live inside admin_campaigns.data and are
  -- managed by /admin/campaign-order. Incoming content never contains
  -- them, so we always carry them forward from the existing row.
...
BEGIN
  ...
  -- Preserve admin-owned ordering keys: existing wins whenever the
  -- incoming payload omits the key or sets it to JSON null.
  FOREACH v_key IN ARRAY v_preserve LOOP
    IF (v_existing ? v_key)
       AND (NOT (v_out ? v_key) OR jsonb_typeof(v_out->v_key) = 'null') THEN
      v_out := jsonb_set(v_out, ARRAY[v_key], v_existing->v_key, true);
    END IF;
  END LOOP;
...
-- ---------- Fixes 2 & 3: last_editor_email + archived guard ----------
...
DECLARE
  ...
  v_allow_unarchive BOOLEAN := COALESCE((plan->'metadata'->>'allow_unarchive')::boolean, false);
...
BEGIN
  ...
  -- Fix 2: resolve editor email once, reuse for every row.
  ...
  BEGIN
      ...
      BEGIN
        ...
        ELSE
          ...
          IF v_action = 'update' THEN
            ...
            END IF;
            -- Fix 3: block silent unarchive. Publish path is the risk;
            -- draft-only writes leave status alone below.
            IF v_existing.status = 'archived' AND v_publish AND NOT v_allow_unarchive THEN
              RAISE EXCEPTION 'archived campaign — enable unarchive opt-in to import';
            END IF;
          ...
          IF v_action = 'new' THEN
            IF v_publish THEN
              INSERT INTO public.admin_campaigns(id,slug,title,status,data,draft_data,content_version,has_unpublished_changes,published_at,updated_by,last_editor_email)
              VALUES (v_campaign_id, v_merged->>'slug', v_merged->>'title','published',v_merged,NULL,1,false,now(),v_uid,v_uemail);
            ...
            ELSE
              INSERT INTO public.admin_campaigns(id,slug,title,status,data,draft_data,content_version,has_unpublished_changes,updated_by,last_editor_email)
              VALUES (v_campaign_id, v_merged->>'slug', v_merged->>'title','draft','{}'::jsonb,v_merged,0,true,v_uid,v_uemail);
            END IF;
    ...
    IF p_mode='dry_run' THEN RAISE EXCEPTION 'DRY_RUN_ROLLBACK' USING ERRCODE='PT001'; END IF;
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      RETURN jsonb_build_object('status','failed','batch_id',v_batch,
                                ...
                                'created',0,'updated',0,'skipped',0,'failed',GREATEST(v_failed,1),
