-- 1) Owner role must pass the staff gate. Additive: only widens.
...
-- 2) Engagement + content performance aggregate (admin-only, bounded).
CREATE OR REPLACE FUNCTION public.analytics_engagement_v16(
  p_from timestamptz,
  p_to   timestamptz
)
RETURNS jsonb
...
REVOKE ALL ON FUNCTION public.analytics_engagement_v16(timestamptz, timestamptz) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.analytics_engagement_v16(timestamptz, timestamptz) TO authenticated, service_role;