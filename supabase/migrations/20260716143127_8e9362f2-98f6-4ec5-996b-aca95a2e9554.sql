...
BEGIN
  ...
  BEGIN
    FOR v_item IN SELECT jsonb_array_elements(v_items) LOOP
      ...
      EXCEPTION
        WHEN sqlstate 'P0004' THEN
          -- Stale version signal: record as a conflict; do NOT re-raise so
          -- we can accumulate a full failure report before aborting the batch.
    ...
    -- Force whole-batch rollback if any item failed or hit a stale conflict.
    -- The RPC contract only permits status=succeeded when every item applied.
  ...
  EXCEPTION
    WHEN sqlstate 'P0001' THEN
      -- Dry run: roll back writes but return as ready.
      NULL;
    WHEN sqlstate 'P0003' THEN
      -- Item-level failure: entire outer subtransaction is rolled back,
      -- so no rows written. Persist a failed batch record for history.
