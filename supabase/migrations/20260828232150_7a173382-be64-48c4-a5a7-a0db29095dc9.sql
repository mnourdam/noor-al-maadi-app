-- ============================================================
-- V16 STREAK DURABILITY — ADDITIVE ONLY
-- Does NOT modify public.record_streak_activity (V15 contract).
-- ============================================================
...
CREATE INDEX IF NOT EXISTS user_streak_days_user_day_idx
  ON public.user_streak_days (user_id, activity_day DESC);

GRANT SELECT, INSERT ON public.user_streak_days TO authenticated;
GRANT ALL ON public.user_streak_days TO service_role;

ALTER TABLE public.user_streak_days ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "own streak days select" ON public.user_streak_days;
CREATE POLICY "own streak days select"
  ON public.user_streak_days FOR SELECT TO authenticated
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "own streak days insert" ON public.user_streak_days;
CREATE POLICY "own streak days insert"
...
-- ============================================================
CREATE OR REPLACE FUNCTION public.record_streak_activity_v16(
  p_source       text DEFAULT NULL,
  p_source_id    text DEFAULT NULL,
  p_activity_day date DEFAULT NULL,
...
BEGIN
  ...
  -- Lock the profile row: all streak math for this user serialises here.
  ...
  -- ---- Activity-day validation (wrong-device-clock protection) ----
  day_in := COALESCE(p_activity_day, today_ry);
  IF day_in > today_ry THEN
    day_in := today_ry;              -- future clock skew: clamp forward-dated to today
  ELSIF day_in < min_day THEN
    rejected := true;                -- too stale to be trusted: never recorded
    reject_reason := 'activity_day_out_of_window';
  END IF;

  -- ---- One-time legacy bridge: preserve pre-V16 streaks ----
  -- Materialise the run implied by profiles.streak/last_streak_day so the
  -- ledger-derived streak can never regress an existing player.
  ...
  -- ---- Derive the streak from the ledger (order-independent) ----
  WITH g AS (
    SELECT activity_day,
  ...
  -- Never regress a pre-existing streak value on upgrade.
  ...
  -- ---- Milestones: identical protections to V15 (unique claim row) ----
  FOR ms IN
    SELECT * FROM (VALUES
...
REVOKE ALL ON FUNCTION public.record_streak_activity_v16(text, text, date, text) FROM PUBLIC;
