CREATE OR REPLACE FUNCTION public.admin_set_artifact_rarity(_ids uuid[], _rarity text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF NOT public.is_user_manager() THEN
    RAISE EXCEPTION 'Forbidden: admin role required (admin_set_artifact_rarity)' USING ERRCODE = '42501';
  END IF;
