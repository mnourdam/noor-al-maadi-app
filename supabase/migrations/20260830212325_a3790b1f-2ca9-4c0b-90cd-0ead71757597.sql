-- V16 — notification audience authorization fix (ADDITIVE, non-destructive).
-- Root cause: the audience resolvers required has_role(uid,'admin') only,
-- so the IRTH owner account (role = 'owner') was rejected with 'forbidden'.

CREATE OR REPLACE FUNCTION public.is_notification_audience_staff(_user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.is_notification_audience_staff(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.is_notification_audience_staff(uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.is_notification_audience_staff(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_notification_audience_staff(uuid) TO service_role;
...
BEGIN
  ...
  IF NOT public.is_notification_audience_staff(v_uid) THEN RAISE EXCEPTION 'forbidden'; END IF;
