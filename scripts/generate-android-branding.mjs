...
// Splash-only foreground: fills ~96dp of the 108dp canvas (no adaptive-icon
// safe-zone padding). The Android 12+ SplashScreen API re-frames the icon
// inside its own 240dp/192dp mask, so a vector scaled to the 66dp adaptive
// safe zone reads as a small boxed app-icon plate. Filling ~96dp keeps the
// mark centered and edge-to-edge inside the system splash window with only
// clean transparent pixels around it.
function svgToSplashIcon(svg) {
  const { w, h } = parseViewBox(svg);
  const fill = 96; // dp inside the 108dp splash canvas
  ...
  return `<?xml version="1.0" encoding="utf-8"?>
<!-- AUTO-GENERATED by scripts/generate-android-branding.mjs — do not edit.
     Splash-only vector: fills ~96dp of the 108dp canvas so the Android 12+
     SplashScreen API renders the mark edge-to-edge inside its icon window
     rather than as a small boxed launcher plate. Do NOT reference this
     drawable from any adaptive-icon XML — it lacks safe-zone padding and
     will clip inside a launcher mask. -->
<vector xmlns:android="http://schemas.android.com/apk/res/android"
    android:width="108dp"
    android:height="108dp"
    android:viewportWidth="108"
