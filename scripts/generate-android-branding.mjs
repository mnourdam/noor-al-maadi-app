...
function writeVectors() {
  ...
  // Splash icon for the Android 12+ SplashScreen API.
  //
  // The OS treats `windowSplashScreenAnimatedIcon` as an adaptive-icon-
  // shaped drawable (108dp canvas, mark inside the 66dp visible safe
  // zone). Feeding it a plain full-bleed vector caused several OEMs to
  // draw a default dark circular/square icon plate behind the mark —
  // that was the "black square" reported around the Irth logo on cold
  // start. Declaring the splash icon as an adaptive-icon with a fully
  // transparent background suppresses the plate on every device we
  // tested and matches the Material 3 splash guidance.
  //
  // - `drawable-anydpi-v26/ic_splash_icon.xml` → adaptive icon
  //   (transparent bg + already-inset foreground). API 26+.
  // - `drawable/ic_splash_icon.xml` → vector fallback for API < 26.
  //   Pre-API-26 devices do not use the SplashScreen API anyway; the
  //   fallback exists only so the resource reference resolves.
  const splashAdaptive = `<?xml version="1.0" encoding="utf-8"?>
<!-- AUTO-GENERATED by scripts/generate-android-branding.mjs — do not edit. -->
...
`;
  write(join(RES, "drawable-anydpi-v26", "ic_splash_icon.xml"), splashAdaptive);

  // Vector fallback (< API 26). Uses the inset adaptive foreground so
  // the mark keeps proper safe-area padding even on old devices.
