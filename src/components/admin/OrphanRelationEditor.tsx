...
// orphan entity. Nothing is auto-created — every relation shown is
// only a suggestion until the admin approves it. Approved slugs are
...
import {
  ...
  MapPin, Globe, Landmark as LandmarkIcon, Tag, Swords,
} from "lucide-react";
...
// Which target-types are meaningful for each source-type. Used to
// bias suggestions and to label the "allowed link kinds" strip.
...
type Suggestion = {
  row: Row;
  reasons: string[];   // human-readable reason chips
  score: number;
};
...
export function OrphanRelationEditor({
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set()); // slugs to add
  const [query, setQuery]       = useState("");
  const [saving, setSaving]     = useState(false);
  const [err, setErr]           = useState<string | null>(null);
  const [campaignSlugs, setCampaignSlugs] = useState<Set<string>>(new Set());

  // Look up campaigns that reference this entity, then pull every OTHER
  // slug the campaign references → "same campaign" suggestion bucket.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
  ...
  // Displayable pool minus self and already-linked entities.
  const pool = useMemo(() => {
  ...
  const suggestions = useMemo<Suggestion[]>(() => {
    ...
    for (const r of pool) {
      ...
      if (campaignSlugs.has(r.slug))                       bump(r, "نفس الحملة", 100 + kindBoost);
      if (state && (rState === state || r.slug === state)) bump(r, "نفس الدولة", 60 + kindBoost);
      if (city  && (rCity  === city  || r.slug === city))  bump(r, "نفس المدينة", 55 + kindBoost);
        ...
        <div className="flex items-start justify-between gap-3 border-b border-amber-500/20 bg-gradient-to-l from-slate-900 to-slate-950 px-5 py-4">
          <div className="min-w-0">
            <div className="mb-1 flex items-center gap-2 text-[11px] tracking-widest text-amber-300">
              <Link2 className="size-3.5" /> ربط كيان يتيم
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          <div className="mb-4">
            <label className="mb-1.5 flex items-center gap-1.5 text-[11px] text-slate-400">
              <Search className="size-3.5" /> ابحث عن كيان لإضافته يدويًا
            </label>
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="اسم أو slug…"
            ...
            />
            {query.trim() && (
              <ul className="mt-2 max-h-52 space-y-1 overflow-y-auto rounded-lg border border-slate-800/60 bg-slate-950/60 p-1">
