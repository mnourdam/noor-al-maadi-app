 ...
 */
export function getStableStorageKey(url) {
    if (!url || typeof url !== "string")
        return null;
...
async function toObjectUrl(response) {
    try {
...
async function fetchAndCache(url, cache, cacheKey = url) {
    const sameOrigin = (() => {
        try {
    ...
    try {
        const init = sameOrigin ? {} : { mode: "no-cors", credentials: "omit" };
 ...
 */
export async function resolveKeyedImageUrl(cacheKey, getFetchUrl) {
    if (!cacheKey)
        return null;
 ...
 */
export async function prefetchKeyedImages(entries) {
    const cache = await openCache();
    if (!cache)
        return;
    const online = typeof navigator === "undefined" || navigator.onLine !== false;
    ...
    async function worker() {
        while (i < list.length) {
            ...
            try {
                ...
                if (url)
                    await fetchAndCache(url, cache, e.key);
            }
            catch { /* ignore */ }
        }
    }
    await Promise.all(Array.from({ length: Math.min(PREFETCH_CONCURRENCY, list.length) }, () => worker()));
}
/**
    ...
    async function worker() {
        while (i < list.length) {
            ...
            while (retries <= PREFETCH_RETRY_LIMIT) {
                ...
                retries++;
                if (retries <= PREFETCH_RETRY_LIMIT)
...
export function collectImageUrls(root, out = new Set()) {
    const stack = [root];
    ...
    while (stack.length) {
        ...
        if (typeof node === "string") {
            if (isLikelyImage(node) && /^https?:\/\//i.test(node))
                out.add(node);
            continue;
        }
        ...
        if (typeof node === "object") {
            for (const [k, v] of Object.entries(node)) {
                if (typeof v === "string") {
                    if ((IMAGE_KEY.test(k) || isLikelyImage(v)) && /^https?:\/\//i.test(v))
                        out.add(v);
                }
                else {
                    stack.push(v);
...
export function useCachedImageSrc(url) {
    const [src, setSrc] = useState(url && (url.startsWith("data:") || url.startsWith("blob:")) ? url : null);
    useEffect(() => {
        let alive = true;
        let created = null;
        if (!url) {
