...
const cache = new Map();
function keyOf(row) {
    const pv = Number.isFinite(row.processing_version)
...
export async function signStoryMediaUrl(row) {
    ...
    promise.then((v) => { if (v === null)
        cache.delete(k); }).catch(() => cache.delete(k));
    return promise;
}
/** Invalidate any cached signed URL for this row (call after re-upload). */
export function invalidateStoryMediaUrl(row) {
 ...
 */
export function storyMediaCacheKey(row) {
    if (!row?.storage_bucket || !row?.storage_path)
        return null;
    const base = import.meta.env?.VITE_SUPABASE_URL ?? "irth://storage";
 ...
 */
export async function resolveCachedStoryMediaUrl(row) {
...
export async function prefetchStoryMediaRows(rows) {
...
export function useStoryMediaUrl(row) {
