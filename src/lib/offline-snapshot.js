...
import { loadSnapshot, saveSnapshot, MIN_PUBLIC_ENCYCLOPEDIA_ROWS, SNAPSHOT_SCHEMA_VERSION, } from "./offline-storage";
const LEGACY_TO_COLLECTION = {
...
const STORY_MANIFEST_KEYS = new Set([
    "stories", "story_scenes", "story_media",
]);
let _manifestPromise = null;
async function fetchStoryManifest() {
    if (_manifestPromise)
        return _manifestPromise;
    _manifestPromise = (async () => {
        try {
            const { data, error } = await supabase.rpc("stories_snapshot_manifest_v2", { p_include_on_demand: false });
            if (error) {
...
function pickManifestSlice(key, m) {
    if (!m.ok)
        return [];
    if (key === "stories")
        return Array.isArray(m.stories) ? m.stories : [];
    if (key === "story_scenes")
        return Array.isArray(m.story_scenes) ? m.story_scenes : [];
    if (key === "story_media")
        return Array.isArray(m.story_media) ? m.story_media : [];
    return [];
}
async function fetchCollection(def) {
    ...
    if (def.key === "encyclopedia_entities") {
        ...
        if (expectedTotal < MIN_PUBLIC_ENCYCLOPEDIA_ROWS) {
            throw new Error(`[snapshot] encyclopedia_entities: live count ${expectedTotal} is below required floor ` +
    ...
    if (typeof expectedTotal === "number" && out.length !== expectedTotal) {
        const msg = `[snapshot] ${def.table}: fetched ${out.length}/${expectedTotal} rows ` +
            `— full fetch did not match expected count, refusing to persist data`;
        console.warn(msg);
        throw new Error(msg);
    }
    console.info(`[snapshot] ${def.table}: fetched ${out.length} rows` +
 ...
 * caller must fall back to a full fetch.
 */
async function fetchCollectionSince(def, since) {
    if (NO_UPDATED_AT.has(def.key))
        return null;
    const PAGE = 500;
    const out = [];
 ...
 */
async function fetchCollectionExpectedCount(def) {
...
function pruneOfflineRow(def, row) {
    ...
    if (def.key === "admin_campaigns") {
        ...
        const { draft_data: _draftData, last_editor_email: _lastEditorEmail, updated_by: _updatedBy, ...playerRow } = row;
        return playerRow;
    }
...
function pruneOfflineRows(def, rows) {
    ...
    try {
        ...
        if (typeof window !== "undefined") {
            if ("requestIdleCallback" in window) {
                window.requestIdleCallback(() => void warmTask(), { timeout: 10000 });
            }
            else {
 ...
 */
async function warmSnapshotImageCache(collections) {
    try {
        ...
        const priorityIds = collectPriorityMediaIds(collections.stories ?? [], collections.story_scenes ?? []);
 ...
 */
export async function refreshSnapshotIncremental() {
    resetStoryManifestCache();
    const previous = await loadSnapshot();
    ...
    const { fetchContentManifest } = await import("./offline-manifest");
    const serverManifest = await fetchContentManifest();
    const nextCollections = { ...previous.collections };
    const nextCounts = { ...previous.content_counts };
    const manifest = [];
    let totalDeltas = 0;
    for (const def of COLLECTIONS) {
        ...
        try {
            ...
            const serverItem = serverManifest?.find(m => (m.collection === 'campaigns_public' && def.key === 'admin_campaigns') ||
        ...
        if (merged.length === 0 && prevRows.length > 0)
            merged = prevRows;
...
export async function loadBundledSnapshot() {
    const urls = new Set([BUNDLED_SNAPSHOT_URL]);
    try {
    ...
    for (const url of urls) {
        try {
            ...
            if (!j || typeof j !== "object" || !j.collections)
                continue;
            const snap = { ...j, source: "bundled" };
            const { validateSnapshot } = await import("./offline-snapshot-validate");
            if (!validateSnapshot(snap).ok)
                continue;
            return snap;
        }
        catch { /* try next URL */ }
...
export async function getCollection(key) {
    ...
    if (local?.collections?.[key]?.length)
        return local.collections[key];
    const bundled = await loadBundledSnapshot();
    if (bundled?.collections?.[key]?.length)
        return bundled.collections[key];
    const def = COLLECTIONS.find((c) => c.key === key);
    if (def && typeof navigator !== "undefined" && navigator.onLine !== false) {
        try {
            return (await fetchCollection(def));
        }
        catch { /* ignore */ }
    }
    return [];
...
export async function getContent(type, legacyFallback) {
    const rows = await getCollection(LEGACY_TO_COLLECTION[type]);
    if (rows.length > 0)
        return rows;
    if (legacyFallback) {
        try {
...
function hasRequiredSnapshotContent(snap) {
    if (!snap?.collections)
        return false;
...
export async function bootstrapOfflineSync(opts = {}) {
    ...
    try {
        ...
        void generateAndStoreSnapshot().catch((e) => console.warn("[offline-sync] background convergence failed:", e));
...
export const REQUIRED_COLLECTION_KEYS = COLLECTIONS
    .filter((c) => c.required)
    .map((c) => c.key);
