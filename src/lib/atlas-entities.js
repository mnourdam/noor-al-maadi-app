...
export const LC1_ATLAS_VISIBLE_KINDS = new Set([
    "region",
    "place",
    "battle",
]);
export function isLc1VisibleAtlasKind(kind) {
    return !!kind && LC1_ATLAS_VISIBLE_KINDS.has(kind);
}
export function filterLc1AtlasRows(rows) {
    return rows.filter((r) => isLc1VisibleAtlasKind(r.kind));
}
export const ATLAS_ENTITY_STATUSES = [
    "draft",
    "review",
    "published",
    "retired",
];
export const KIND_LABEL_AR = {
    place: "موقع",
    battle: "معركة",
    event: "حدث",
    figure_marker: "شخصية",
 ...
 */
export const ATLAS_PUBLIC_COLUMNS = "id,slug,kind,name_ar,name_en,aps_x,aps_y,aps_verified,aps_verified_at," +
...
export async function listPublishedAtlasEntities() {
    try {
...
export async function listAllAtlasEntities() {
    const { data, error } = await supabase
        .from("atlas_entities")
        .select(ATLAS_PUBLIC_COLUMNS)
...
export async function getAtlasEntity(id) {
    const { data, error } = await supabase
        .from("atlas_entities")
        .select(ATLAS_PUBLIC_COLUMNS)
        .eq("id", id)
...
export async function createAtlasEntity(input) {
...
export async function updateAtlasEntity(id, patch) {
    const { data, error } = await supabase
        .from("atlas_entities")
        .update(patch)
        .eq("id", id)
...
export async function verifyAtlasEntity(id, reviewerId) {
    return updateAtlasEntity(id, {
        aps_verified: true,
        aps_verified_by: reviewerId,
    });
}
export async function unverifyAtlasEntity(id) {
    return updateAtlasEntity(id, { aps_verified: false });
}
export async function setAtlasEntityStatus(id, status) {
    return updateAtlasEntity(id, { status });
}
export async function deleteAtlasEntity(id) {
...
export function suggestSlug(nameAr, nameEn) {
