...
export function stripHonorifics(input: string): string {
  let s = input || "";
  for (const re of HONORIFICS) s = s.replace(re, " ");
...
export function entityNameKeys(args: {
  ...
  if (args.title) out.add(normalizeArabicName(args.title));
  if (args.subtitle) out.add(normalizeArabicName(args.subtitle));
  const m = args.metadata ?? {};
  const aliasArrays: any[] = [m.aliases, m.also_known_as, m.alt_names, m.names];
  for (const arr of aliasArrays) {
    if (Array.isArray(arr)) for (const a of arr) if (typeof a === "string") out.add(normalizeArabicName(a));
  }
  out.delete("");
