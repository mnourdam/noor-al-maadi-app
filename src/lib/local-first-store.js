...
import { loadSnapshot, saveSnapshot, MIN_PUBLIC_ENCYCLOPEDIA_ROWS } from "./offline-storage";
import { normalizeArabicName } from "./arabic-normalize";
import { isDividerRow, partitionCampaignRows, } from "./campaigns/entities";
let _ready = false;
let _readyPromise = null;
let _snapshot = null;
const REQUIRED_LOCAL_COLLECTIONS = ["encyclopedia_entities", "admin_campaigns"];
function hasRequiredContent(snap) {
    if (!snap?.collections)
        return false;
    return REQUIRED_LOCAL_COLLECTIONS.every((key) => {
        ...
        if (key === "encyclopedia_entities")
            return rows.length >= MIN_PUBLIC_ENCYCLOPEDIA_ROWS;
        return true;
    });
}
const encyclopediaById = new Map();
const encyclopediaBySlug = new Map(); // slug → list across types
const encyclopediaByTypeSlug = new Map(); // `${type}::${slug}` → row
const encyclopediaByType = new Map();
const encyclopediaByAlias = new Map();
const encyclopediaByLegacyId = new Map();
...
const encyclopediaByNormName = new Map();
let encyclopediaAll = [];
const atlasPublished = [];
...
let campaignsAll = [];
let campaignDividerRows = [];
const investigationsBySlug = new Map();
let investigationsAll = [];
const tihByMonthDay = new Map(); // `${m}-${d}` → list
let dailyFactsAll = [];
...
function indexEncyclopedia(rows) {
    ...
    for (const r of encyclopediaAll) {
        if (r.id)
            encyclopediaById.set(r.id, r);
        if (r.slug) {
            const list = encyclopediaBySlug.get(r.slug) ?? [];
            list.push(r);
            encyclopediaBySlug.set(r.slug, list);
            if (r.entity_type)
                encyclopediaByTypeSlug.set(`${r.entity_type}::${r.slug}`, r);
        }
        if (r.entity_type) {
...
function indexAtlas(rows) {
    atlasPublished.length = 0;
    for (const r of rows) {
        if (r?.status === "published" && r?.aps_verified)
            atlasPublished.push(r);
    }
}
function indexCampaigns(rows) {
    campaignsById.clear();
    campaignsBySlug.clear();
    const published = rows.filter((r) => r && r.status === "published");
    const split = partitionCampaignRows(published);
    campaignsAll = split.campaigns;
    campaignDividerRows = published.filter((r) => isDividerRow(r));
    for (const r of campaignsAll) {
        if (r.id)
            campaignsById.set(r.id, r);
        if (r.slug)
            campaignsBySlug.set(r.slug, r);
    }
}
function indexInvestigations(rows) {
    investigationsBySlug.clear();
    investigationsAll = rows.filter((r) => r && r.enabled !== false);
    for (const r of investigationsAll) {
        if (r.slug)
            investigationsBySlug.set(r.slug, r);
    }
}
function indexTih(rows) {
    tihByMonthDay.clear();
    for (const r of rows) {
...
function indexDailyFacts(rows) {
    dailyFactsAll = rows.filter((r) => r && r.enabled !== false);
}
function indexStories(rows) {
    storiesById.clear();
    storiesBySlug.clear();
    ...
    storiesAll = rows.filter((r) => r &&
        r.status === "published" &&
        !isCampaignIntroRow(r, introIds));
    for (const r of storiesAll) {
        if (r.id)
            storiesById.set(String(r.id), r);
        if (r.slug)
            storiesBySlug.set(String(r.slug), r);
    }
}
function indexScenes(rows) {
    scenesByStory.clear();
    for (const r of rows) {
...
export function localDataVersion() { return _dataVersion; }
/** Subscribe to snapshot (re)application. Returns an unsubscribe fn. */
export function onLocalSnapshotChange(cb) {
    _listeners.add(cb);
    return () => { _listeners.delete(cb); };
}
/** Rebuild every index from a snapshot. Safe to call repeatedly. */
export function applyLocalSnapshot(snap) {
    if (!snap?.collections)
        return;
 ...
 */
export function mergeLocalCampaignRows(rows) {
    if (!Array.isArray(rows) || rows.length === 0)
        return false;
    indexCampaigns(rows);
 ...
 */
export function ensureLocalSnapshotLoaded() {
    if (_ready)
    ...
    _readyPromise = (async () => {
        try {
            ...
            if (typeof console !== "undefined")
                console.warn("[local-first] init failed", e);
        }
        finally {
            _ready = !!_snapshot;
            if (!_ready)
...
export function isLocalReady() { return _ready; }
...
function richnessScore(e) {
    ...
    if (b && typeof b === "object") {
        const bb = b;
        if (Array.isArray(bb.sections))
            s += bb.sections.length * 4;
        if (Array.isArray(bb.timeline))
            s += bb.timeline.length * 3;
        if (Array.isArray(bb.facts))
            s += bb.facts.length;
        if (Array.isArray(bb.sources))
            s += bb.sources.length;
        if (typeof bb.overview === "string")
            s += Math.min(5, Math.floor(bb.overview.length / 200));
        if (typeof bb.introduction === "string")
            s += Math.min(5, Math.floor(bb.introduction.length / 200));
    }
    ...
    if (typeof e.summary === "string" && e.summary.trim().length > 0)
        s += 1;
    if (typeof e.subtitle === "string" && e.subtitle.trim().length > 0)
        s += 1;
    return s;
}
function pickRichest(list, preferType) {
...
export function localEncyclopediaById(id) {
    if (!id)
        return null;
    ...
    if (richnessScore(direct) > 1)
        return direct;
    if (direct.slug) {
        const richer = pickRichest(encyclopediaBySlug.get(direct.slug) ?? [direct]);
        if (richer && richnessScore(richer) > richnessScore(direct))
            return richer;
    }
    return direct;
}
export function localEncyclopediaBySlug(slug, type) {
    if (!slug)
        return null;
    if (type) {
        const hit = encyclopediaByTypeSlug.get(`${type}::${slug}`);
        if (hit && richnessScore(hit) > 1)
            return hit;
    }
    const list = encyclopediaBySlug.get(slug);
    return pickRichest(list ?? [], type ?? null);
}
export function localEncyclopediaByType(type) {
    return encyclopediaByType.get(type) ?? [];
}
export function localEncyclopediaSlugCandidates(slug) {
...
export function localEncyclopediaSameNameSiblings(entity) {
    if (!entity?.entity_type || !entity?.title)
        return [];
...
export function localEncyclopediaAll() { return encyclopediaAll; }
export function localAtlasEntities() { return atlasPublished; }
/** Playable campaigns only — section dividers are NEVER included. */
export function localPublishedCampaigns() { return campaignsAll; }
/** Raw section-divider rows (organizational only). */
export function localCampaignDividerRows() { return campaignDividerRows; }
/** Raw published games (used for Daily Challenges rotation). */
export function localPublishedGames() {
    if (!_snapshot?.collections?.games)
        return [];
    return _snapshot.collections.games.filter(g => g && g.status === 'published');
}
export function localCampaignByIdOrSlug(idOrSlug) {
    if (!idOrSlug)
        return null;
 ...
 */
export function invalidateLocalCampaign(idOrSlug) {
    if (!idOrSlug)
        return;
    ...
    if (!row)
        return;
    if (row.id)
        campaignsById.delete(row.id);
    if (row.slug)
        campaignsBySlug.delete(row.slug);
    campaignsAll = campaignsAll.filter(r => r !== row);
}
export function localInvestigations() { return investigationsAll; }
export function localInvestigationBySlug(slug) {
 ...
 */
export function invalidateLocalInvestigation(idOrSlug) {
    if (!idOrSlug)
        return;
    // We only index by slug for investigations; match either the slug key or
    // the row's id defensively.
    let hit = investigationsBySlug.get(idOrSlug) ?? null;
    if (!hit) {
        for (const r of investigationsAll) {
            if (r.id === idOrSlug || r.slug === idOrSlug) {
                hit = r;
                break;
            }
        }
    ...
    if (!hit)
        return;
    if (hit.slug)
        investigationsBySlug.delete(hit.slug);
    investigationsAll = investigationsAll.filter((r) => r !== hit);
}
export function localTihForMonthDay(month, day) {
    return tihByMonthDay.get(`${month}-${day}`) ?? [];
}
export function localTihAll() {
    const out = [];
    for (const list of tihByMonthDay.values())
        out.push(...list);
    return out;
}
export function localDailyFacts() { return dailyFactsAll; }
...
// ---------- Stories (P5) ----------
export function localStoriesAll() { return storiesAll; }
export function localStoryById(id) {
    if (!id)
        return null;
    return storiesById.get(id) ?? storiesBySlug.get(id) ?? null;
}
export function localStoryScenes(storyId) {
    return scenesByStory.get(storyId) ?? [];
}
export function localStoryMediaById(id) {
    if (!id)
        return null;
    return mediaById.get(id) ?? null;
}
export function localStoryMediaForStory(storyId, referencedIds = []) {
    const out = new Map();
    for (const m of mediaByStory.get(storyId) ?? [])
 ...
 */
export function pruneStoriesToAuthoritative(authoritativeIds) {
    const keep = new Set();
    for (const id of authoritativeIds)
        if (id)
            keep.add(String(id));
    if (keep.size === 0)
        return; // never prune to empty on a bad response
